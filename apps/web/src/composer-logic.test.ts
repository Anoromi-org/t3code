...
describe("canRunStandaloneComposerSlashCommand", () => {
  ...
  it("does not execute slash actions while an expired terminal pill remains attached", () => {
    expect(
      canRunStandaloneComposerSlashCommand({ ...emptyComposer, terminalContextCount: 1 }),
    ).toBe(false);
  });
});
