...
Retain upstream provider-instance selection, Antigravity capabilities, attachment handling, and rewind map reuse. Include file attachments in the standalone slash-action guard. Preserve the fork's focus, interrupt, and file-picker defaults; assign Mod+Alt+Shift+S/C/P to upstream settlement, reference copy, and pinning to avoid collisions without rewriting customized bindings. Test effective resolution for all six actions.
