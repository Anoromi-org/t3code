...
The rebased implementation keeps upstream contextual project settings and the opt-in legacy sidebar: sidebar-v2 reaches Hyprnav from each project settings page, while the legacy project context menu retains its direct scoped action. Provider-setting writes remain environment-scoped and are serialized as durable functional updates so concurrent edits are not lost.

## September modernization

Hyprnav's awaited client saves use upstream's durable client-settings queue, including its protection against overlapping optimistic writes. The four former client-queue tests now exercise `persistClientSettingsDurablePatch` through that implementation. Preserve upstream shared-settings synchronization, hosted-app availability warnings, and shared model preferences when deleting or resetting an environment-local provider. Functional provider updates remain serialized against authoritative responses.
